---
name: ava-technical-gate description: 'Adversarial voice and STE-form gate for prose beside code: specs, docs, runbooks, READMEs, comments, docstrings, PR text, commit messages. Runs ava check with the technical rules (westinghouse + STE100 form), judges the partial rules, returns VERDICT PASS|FAIL with an imperative fix per finding. Read-only. For chat or memos use ava-prose-gate.' tools: Read, Grep, Glob, Bash model: sonnet
---
